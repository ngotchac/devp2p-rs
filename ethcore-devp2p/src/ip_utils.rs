...
pub trait SocketAddrExt {
    ...
    fn is_running_benchmark(&self) -> bool {
        false
    }
    ...
    fn is_socket_reserved(&self) -> bool;
...
impl SocketAddrExt for Ipv4Addr {
    ...
    fn is_socket_reserved(&self) -> bool {
        self.is_unspecified()
            ...
            || self.is_running_benchmark()
            || self.is_future_use()
    }

    fn is_usable_public(&self) -> bool {
        !self.is_socket_reserved() && !self.is_private()
    }
...
impl SocketAddrExt for Ipv6Addr {
    ...
    fn is_usable_public(&self) -> bool {
        !self.is_socket_reserved() && !self.is_unique_local_s()
    }
...
impl SocketAddrExt for IpAddr {
    ...
    fn is_socket_reserved(&self) -> bool {
        match *self {
            IpAddr::V4(ref ip) => ip.is_socket_reserved(),
            IpAddr::V6(ref ip) => ip.is_socket_reserved(),
        }
    }
    ...
    match get_if_addrs() {
        Ok(list) => {
            ...
            for addr in &list {
                ...
                match addr {
                    IpAddr::V4(a) if !a.is_socket_reserved() => {
            ...
            for addr in &list {
                match addr {
                    IpAddr::V6(a) if !a.is_socket_reserved() => {
...
fn ipv4_benchmarking() {
    assert!(!Ipv4Addr::new(198, 17, 255, 255).is_running_benchmark());
    assert!(Ipv4Addr::new(198, 18, 0, 0).is_running_benchmark());
    assert!(Ipv4Addr::new(198, 19, 255, 255).is_running_benchmark());
    assert!(!Ipv4Addr::new(198, 20, 0, 0).is_running_benchmark());
}
