...
use ethcore_io::StreamToken;
use ethkey::crypto::{ecdh, ecies};
use ethkey::{recover, sign, Generator, KeyPair, Public, Random, Secret};
use network::Error;

use crate::connection::{Connection, ConnectionWriter, ConnectionInfo};
...
impl Handshake {
    ...
    pub fn new(
        ...
        Ok(Handshake {
            ...
            connection: Connection::new(token, conn_writer, conn_info),
            originated: false,
    ...
    /// Start a handshake
    pub async fn start(&mut self, host: &HostInfo, originated: bool) -> Result<(), Error> {
        self.originated = originated;
        // @todo Implement Timeout
        // io.register_timer(self.connection.token, HANDSHAKE_TIMEOUT)
        //     .ok();
        if originated {
            self.write_auth(host.secret(), host.id()).await?;
        } else {
    ...
    pub async fn readable(&mut self, host: &HostInfo) -> Result<(), Error> {
        while let Some(data) = self.connection.readable().await? {
            match self.state {
                HandshakeState::New => {},
                HandshakeState::StartSession => {},
                HandshakeState::ReadingAuth => {
                    self.read_auth(host.secret(), &data).await?;
                },
                HandshakeState::ReadingAuthEip8 => {
                    self.read_auth_eip8(host.secret(), &data).await?;
                },
                HandshakeState::ReadingAck => {
                    self.read_ack(host.secret(), &data).await?;
                },
                HandshakeState::ReadingAckEip8 => {
                    self.read_ack_eip8(host.secret(), &data).await?;
                },
            }
            // @todo Implement timeout
    ...
    async fn read_auth(&mut self, secret: &Secret, data: &[u8]) -> Result<(), Error> {
    ...
    async fn read_auth_eip8(&mut self, secret: &Secret, data: &[u8]) -> Result<(), Error> {
        ...
        self.write_ack_eip8().await?;
        Ok(())
    }

    /// Parse and validate ack message
    async fn read_ack(&mut self, secret: &Secret, data: &[u8]) -> Result<(), Error> {
    ...
    async fn read_ack_eip8(&mut self, secret: &Secret, data: &[u8]) -> Result<(), Error> {
        trace!(target: "network", "Received EIP8 handshake auth from {:?}", self.connection.remote_addr_str());
    ...
    /// Sends auth message
    async fn write_auth(&mut self, secret: &Secret, public: &Public) -> Result<(), Error> {
    ...
    async fn write_ack_eip8(&mut self) -> Result<(), Error> {
        ...
        self.connection.send(self.ack_cipher.clone()).await?;
        self.state = HandshakeState::StartSession;
        Ok(())
    }
}
